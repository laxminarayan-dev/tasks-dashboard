...
const CardGroup = ({ taskData = {}, currentSection }) => {
  return (
    <div
      className={`grid-scroll grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-6 overflow-y-auto p-4 ${Object.keys(taskData).length > 0 && taskData[currentSection].length > 0 ? "max-h-[calc(100dvh-12rem)]" : "min-h-[calc(100dvh-12rem)]"} `}
    >
